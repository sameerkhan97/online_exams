...
echo "k8s-common-core remote added"
...
if [[ "$tagName" != *"-dev"* ]];then
    ...
    git tag -a $tagName -m "Dev tag for v${X}.${Y}.${Z} release of Tanzu Framework" $commitSha
fi
...
#If it is the -dev tag or the patch release tag then release branch won't be created
if [[ "$tagName" != *"-dev"* && "$Z" == "0" ]];then
